...
my @diffs=();
my %lookup=();
my %known_logs= ("xcelium" => "$dir/xrun.log", "ius" => "$dir/irun.log", "vcs" => "$dir/simv.log", "questa" => "$dir/run.log");
...
sub scrub {
  ...
  # if this is a factory dump, then deal with the count
  $$logfile =~ s/All types registered with the factory: \d+ total/UVM TEST FACTORY COUNT/mg;
  ...
  if($$logfile =~ /\s+CREATE_STREAM/) {
      ...
      $$logfile =~ s/NAME:inst_id VALUE:(\d+)/NAME:inst_id VALUE:\@H/mg;
  }
