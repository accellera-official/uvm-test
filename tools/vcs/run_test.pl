...
#
# Make sure the version of VCS can run these tests
#
...
if ($vcs_yr > 2014) {
    $vcs_acc = "-debug_access+r+w+nomemcbk";
}
if ($vcs_yr == 2014 && $vcs_mo == 12) { 
    $vcs_acc = "-debug_access+r+w";
}
...
#
sub run_the_test {
  local($testdir, $vcs_opts, $simv_opts, $compat_opts, $_) = @_;
  local $uvm_opts = "+incdir+$uvm_home/src $uvm_home/src/uvm.sv $compat_opts $uvm_home/src/dpi/uvm_dpi.cc -CFLAGS -DVCSMX";

  $vcs = "$vcs_bin -sverilog $vcs_acc -timescale=1ns/1ns  $vcs_opts $uvm_opts test.sv -l vcs.log  +lint=all";
